(() => {
      ...
      // Links cortos de Google Maps (maps.app.goo.gl / goo.gl/maps) no se
      // pueden seguir desde el navegador (CORS). Se resuelven en el servidor
      // vía una Supabase Edge Function ("resolve-maps-link"), de forma
      // silenciosa y sin bloquear el guardado del cliente: si la función no
      // está desplegada o falla, el link igual se guarda tal cual lo pegó el
      // usuario y el geocoding por texto (Nominatim) sigue de respaldo.
        ...
        for(const addr of (c.addresses||[])){
          ...
          try{
            ...
            if(!error && data?.lat!=null && data?.lng!=null){
              addr.lat=data.lat; addr.lng=data.lng; addr.geoSrc=`${addr.maps}|${addr.address||''}`;
              changed=true;
            }
      ...
      function computePresenceCounts(presenceState){
        ...
        Object.values(presenceState||{}).forEach(metas=>{
          (metas||[]).forEach(m=>{ if(counts[m.role]!==undefined) counts[m.role]++; });
        });
      ...
      const PREMIUM_DEFAULT_LOCKED = {notes:true,payroll:true,inventory:true,audit:true,delivery:false,returnDate:true,weeklySchedule:true,exportDiets:true,clientPortal:true};
      ...
      // --- Direcciones múltiples (c.addresses[] + c.activeAddressId) ---
      // Migra clientes con el formato viejo (address1/maps/address2/maps2) a
      // la lista c.addresses[] la primera vez que se cargan. Idempotente: si
      // el cliente ya tiene c.addresses no hace nada. No borra los campos
      // viejos por si algo falla (quedan sin usarse).
      ...
      function initAddressEditor(form, addresses, activeId){
        ...
        render();
        addBtn.onclick=()=>{ list.push({id:uid('a')}); render(); };
      ...
      function initScheduleEditor(form, rows){
        ...
        addBtn.onclick=()=>{ list.push({days:[],addressId:currentAddressOptions()[0]?.id||''}); render(); };
      ...
      function renderSettings(){
        ...
        $('#settings-page').innerHTML=pageHead('Configuración','Preferencias visuales, columnas y respaldo de datos.')+`<div class="two-col">
          ...
          ${isSuperAdmin()?`<div class="card card-pad stack">
            ...
            <div class="stack" style="gap:6px">${PREMIUM_LOCKABLE_PAGES.map(([key,label])=>`<label style="display:flex;align-items:center;gap:8px;font-weight:400"><input type="checkbox" data-premium-page="${key}" ${isPagePremiumLocked(key)?'checked':''}> ${esc(label)}</label>`).join('')}</div>
          </div>`:''}
        </div>`;
        $('#my-theme').onchange=e=>{saveUserTheme(e.target.value);render();};
        $$('[data-column]').forEach(input=>input.onchange=()=>{const key=input.dataset.column,hidden=userColPrefs().hiddenColumns;saveHiddenColumns(input.checked?hidden.filter(x=>x!==key):[...hidden,key]);notice('Columnas actualizadas.');});
      ...
      document.addEventListener('click',e=>{const action=e.target.closest('[data-action]')?.dataset.action,id=e.target.closest('[data-action]')?.dataset.id,kind=e.target.closest('[data-action]')?.dataset.kind;if(!action)return;const map={
        ...
        const ACTION_PERMS={
          ...
          "add-plan":()=>canEditPage('plans'),"edit-plan":()=>canEditPage('plans'),"delete-plan":()=>canEditPage('plans'),"remove-plan-photo":()=>canEditPage('plans'),
      ...
      (() => {
        ...
        // Ahora es por dirección (addr), no por cliente completo.
        ...
        // Resuelve coordenadas de la dirección "efectiva" de cada cliente para
        // la fecha dada. Las coordenadas se guardan en el objeto de esa
        // dirección (c.addresses[i].lat/lng/geoSrc), no a nivel de cliente,
        // porque cada dirección tiene su propia ubicación.
        async function resolveCoords(list, date){
          ...
          for (const {addr} of pending){
            let coords=extractCoords(addr.maps) || extractCoords(addr.address);
