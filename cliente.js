(()=>{
      ...
      function render(message='',error=false,showSupport=false){const date=workDate(),next=nextWorkDay(date),st=stateFor(date),p=plan(),remaining=Math.max(0,n(client.paidDays)-n(client.consumedDays)),included=client.items&&Object.keys(client.items).length?client.items:p?.items||{},theme=clientTheme();document.documentElement.dataset.bsTheme=theme==='night'?'dark':'light';document.documentElement.dataset.theme=theme;
        ...
        <div class="row g-2 mb-3">
          <div class="col-sm-6"><input class="btn-check" type="radio" name="pause-mode" id="pause-mode-open" checked><label class="pause-mode-card border d-block h-100 p-3" for="pause-mode-open"><span class="d-block fw-semibold">⏸ Pausa sin fecha</span><span class="d-block small text-secondary mt-1">Reactivas cuando quieras, avisándonos por acá o por WhatsApp.</span></label></div>
          <div class="col-sm-6"><input class="btn-check" type="radio" name="pause-mode" id="pause-mode-scheduled"><label class="pause-mode-card border d-block h-100 p-3" for="pause-mode-scheduled"><span class="d-block fw-semibold">📅 Con fecha de retorno</span><span class="d-block small text-secondary mt-1">Se reactiva sola el día que elijas, sin que tengas que avisar.</span></label></div>
        </div>
      ...
      async function requestPause(){const now=new Date();if(now.getHours()>=22){render('Ya pasó el horario de pausa automática. Ponte en contacto con Atención al Cliente.',true,true);return;}const next=nextWorkDay(workDate()),scheduled=$('#pause-mode-scheduled')?.checked,returnDate=scheduled?$('#return-date').value:'';if(scheduled&&!returnDate){render('Selecciona una fecha de retorno para esta opción.',true);return;}if(returnDate&&returnDate<=next){render('La fecha de retorno debe ser posterior al día pausado.',true);return;}
